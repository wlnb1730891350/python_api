...
@ddt
class TestAddCases(unittest.TestCase,BaseTest):
    excel = HandleExcel(os.path.join(DATAS_DIR, 'apicases.xlsx'), 'add_testcase')
    ...
    def test_add(self,item):
        ...
        item['data']=replace_data(item['data'],TestAddCases)
        params=eval(item['data'])
        method=item['method'].lower()
        expected=eval(item['expected'])
        if item['check_sql']:
            sql = "SELECT * FROM test.tb_testcases WHERE interface_id='{}'".format(self.interfaces_id)
